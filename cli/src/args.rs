            ...
            while let Some(arg) = argv.pop_front() {
                match arg.as_encoded_bytes() {
                    ...
                    b"-m" | b"--mode" => match argv.pop_front() {
                        None => {
                            return Err(Self::exit_arg_invalid(
                                "no argument provided for -m/--mode",
                            ));
                        }
                    ...
                    b"-n" | b"--name" => match argv.pop_front() {
                        None => {
                            return Err(Self::exit_arg_invalid(
                                "no argument provided for -n/--name",
                            ))
                        }
                    ...
                    b"-f" | b"--file" => match argv.pop_front() {
                        None => {
                            return Err(Self::exit_arg_invalid(
                                "no argument provided for -f/--file",
                            ));
                        }
        ...
        fn get_negation(
            ...
            let negated_expr: MatchExpression = match expr_args.pop_front().ok_or_else(|| {
                ...
                ExprArg::PrimitivePredicate(predicate) => {
                    ...
                    MatchExpression::Negated(Box::new(MatchExpression::PrimitivePredicate(
                        predicate,
                    )))
                }
                ExprArg::Op(op) => {
                ...
                match arg.as_encoded_bytes() {
                    ...
                    b"--compression-method" => {
                        let method_arg = argv.pop_front().ok_or_else(|| {
                            Extract::exit_arg_invalid(
                                "no argument provided for --compression-method",
                            )
                        })?;
                        ...
                        top_exprs.push_arg(ExprArg::PrimitivePredicate(
                            Predicate::CompressionMethod(method),
                        ));
                    }
                    b"--max-depth" => {
                    ...
                    b"-m" => {
                        let pattern: String = argv
                            .pop_front()
                            .ok_or_else(|| {
                                Extract::exit_arg_invalid("no argument provided for -m")
                            })?
                            .into_string()
                            .map_err(|pattern| {
                                Extract::exit_arg_invalid(&format!(
                        ...
                        let comp_sel = ComponentSelector::default();
                        let pat_sel = PatternSelector::default();
                        top_exprs.push_arg(ExprArg::PrimitivePredicate(Predicate::Match(
                            MatchArg {
                                comp_sel,
                                pat_sel,
                                pattern,
...
content transform. add -x/--extract to construct a complete entry spec"
    ...
    impl CommandFormat for Extract {
        ...
        const USAGE_LINE: &'static str =
            "[-h|--help] [OUTPUT-FLAGS] [ENTRY-SPEC]... [--stdin|[--] ZIP-PATH...]";
...
pat-sel  = glob		[DEFAULT] (interpret as a shell glob)
         ...
         = <pat-sel>:i	(use case-insensitive matching for the given pattern)
                        ...
                        let pattern = argv
                            ...
                            .ok_or_else(|| {
                                Self::exit_arg_invalid(
                                    "no <pattern> argument provided for --transform",
                                )
                            })?
                            .into_string()
                            .map_err(|pattern| {
