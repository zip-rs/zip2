...
impl ZipArchiveMetadata {
    ...
    ///
    /// Unlike [`ZipArchive::by_index`], this only reads the already-parsed central directory,
    /// so it needs neither the underlying reader nor `&mut` access to the archive.
    ///
    /// # Errors
    ///
    /// Returns [`ZipError::FileNotFound`] if `index` is out of bounds.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// writer.start_file("hello.txt", SimpleFileOptions::default())?;
    /// writer.write_all(b"Hello, world!")?;
    /// let archive = zip::ZipArchive::new(writer.finish()?)?;
    ///
    /// let metadata = archive.metadata();
    /// for i in 0..metadata.len() {
    ///     let entry = metadata.entry(i)?;
