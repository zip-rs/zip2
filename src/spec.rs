    ...
    pub fn find_and_parse<T: Read + Seek>(
        ...
        while pos >= nominal_offset {
            ...
            for i in 0..=buffer.len() - HEADER_SIZE {
                ...
                if bufreader.read_u32_le()? == ZIP64_CENTRAL_DIRECTORY_END_SIGNATURE {
                    has_end_sig = true;
                    let archive_offset = pos + i as u64 - nominal_offset;

                    let _record_size = bufreader.read_u64_le()?;
                    // We would use this value if we did anything with the "zip64 extensible data sector".

                    let version_made_by = bufreader.read_u16_le()?;
                    let version_needed_to_extract = bufreader.read_u16_le()?;
                    let disk_number = bufreader.read_u32_le()?;
                    let disk_with_central_directory = bufreader.read_u32_le()?;
                    let number_of_files_on_this_disk = bufreader.read_u64_le()?;
                    let number_of_files = bufreader.read_u64_le()?;
                    let central_directory_size = bufreader.read_u64_le()?;
                    let central_directory_offset = bufreader.read_u64_le()?;
